...
$WidgetName    = "feature-identify"   # widget folder name (must match EB folder + repo subfolder)
$RepoName      = "feature-identify-widget"
...
try {
    ...
    if ($Release -ne "") {
        if (-not $gh) {
            ...
            $notes = "Download $WidgetName.zip, extract, and drop the $WidgetName folder into client\your-extensions\widgets. Then run the standard client install (pnpm on EB 1.21+) and restart."
